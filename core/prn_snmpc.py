...
# ----------------------------------------------------------------------------
# import modules
from functions import snmp_func as snmp_func
...
def prn_snmp_chk(prn_file, snmp_file):
    ...
    prn_csv = csv_func.csv_import(prn_file)  # Define Printer CSV Input File.

    snmp_csv = csv_func.csv_import(snmp_file)  # Define SNMP CSV Input File.
    ...
    for printer in prn_csv:  # start looping through all class methods per printer.
        ...
        if prn_tri in snmp_passwd.keys():
            ...
            csv_func.prn_snmpchk_csv_output(
...
def env_chk(prn_file, snmp_file):
    ...
    csv_func.csv_chk(prn_file)  # Check csv file is present and available.
    ...
    csv_func.csv_chk(snmp_file)  # Check csv file is present and available.
    ...
    csv_func.csv_header('output', 'prn_snmpchk_output', '.csv', 'prn_snmpc')
...
def error_msg():
      ...
      log_func.logw('snmpc_log', error_msg)
