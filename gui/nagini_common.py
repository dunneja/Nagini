...
def popup_warning(function, input, passwd, vendor):
    ...
    if popup == 'Yes':
        ...
        if prn_op == prn_conn_c:
            ...
            threading.Thread(target=prn_conn_chk, args=(
                value, ini_file), daemon=True).start()
        elif prn_op == prn_snmp_c:
            #prn_snmp_chk(value, snmp_passwd_file)
            threading.Thread(target=prn_snmp_chk, args=(
                value, ini_file, snmp_passwd_file), daemon=True).start()
        elif prn_op == prn_data_c:
            #prn_data_col(value, ini_file, vendor)
            threading.Thread(target=prn_data_col, args=(
                value, ini_file, vendor), daemon=True).start()
        else:
