...
def prn_tool_gui():
    ...
    layout = [[sg.Menu(menu_def, tearoff=False)],
              ...
              [sg.Text('Printer CSV:', size=(11, 1)), sg.Input(size=(49, 1), default_text='conf/printers.csv',
                                                               key='_INPUT_'), sg.FileBrowse(file_types=(("CSV Files", "*.csv"),),)],
              [sg.Text('Password CSV:', size=(11, 1), key='_CSVPTXT_', visible=False), sg.Input(size=(49, 1), default_text='conf/passwords.csv',
                                                                                                key='_CSVPASSWD_', visible=False), sg.FileBrowse(file_types=(("CSV Files", "*.csv"),), visible=False, key='_CSVPFB_')],
              [sg.Radio('Printer Connection Checker', 'Radio1', default='True', key='_PRNCONNCHK_', enable_events=True), sg.Radio('SNMP Passwd Checker',
                                                                                                                                  'Radio1', enable_events=True, key='_PRNSNMPCHK_'), sg.Radio('SNMP Data Collector', 'Radio1', key='_PRNDATACOL_', enable_events=True), ],
    ...
    while True:
        ...
        if event == 'Start' and values['_PRNCONNCHK_'] == True:
            ...
            popup_warning('prn_conn_chk', prn_conn_chk_input,
                          os.devnull, vendor)
        if event == 'Start' and values['_PRNDATACOL_'] == True:
            window['_OUTPUT_']('')
            prn_data_col_input = values['_INPUT_']
            popup_warning('prn_data_col', prn_data_col_input,
                          os.devnull, vendor)
        if event == 'Start' and values['_PRNSNMPCHK_'] == True:
            ...
            popup_warning('prn_snmp_chk', prn_snmp_chk_prn_input,
                          prn_snmp_chk_passwd_input, vendor)
        if event == 'Clear':
            window['_OUTPUT_']('')
        if event == "Paste":
